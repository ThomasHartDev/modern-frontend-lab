...
- Core Web Vitals budget: `/cwv` measures LCP, INP, and CLS against the Chrome "good" cuts (2.5s / 200ms / 0.1). Pure metric math covers final LCP, CLS session windows (1s gap, 5s cap), and INP (max under 50 samples, ~98th percentile at scale). A slow unreserved hero fails the budget; the fixed reserved hero passes.
